 ...
 * - Reward / Tax Reason (thnaks to mattlynch)
...
class KarmaBank extends Gdn_Plugin {
    ...
    public function AddMeta($Name, $Explanation){
    ...
    public static function OperationEquals($MetaValue,$Target,$Condition,$User,$LastTrans,$Options){
		return $MetaValue == $Target;
	}
	
	public static function OperationEvery($MetaValue,$Target,$Condition,$User,$LastTrans,$Options){
		return $MetaValue % $Target == 0;
	}
	
	public static function OperationDiffEquals($MetaValue,$Target,$Condition,$User,$LastTrans,$Options){
    ...
    */
    public function ProfileController_KarmaBank_Create($Sender,$Args){
        ...
        list($Offset, $Limit) = OffsetLimit(array_key_exists(2,$Args)?$Args[2]:0,C('Plugins.KarmaBank.PageLimit',20));
   ...
   public function PostController_BeforeCommentRender_Handler(&$Sender) {
	...
	protected function ShowBalance(&$Sender) {
		...
		for($i=0;$i<count($levelName);$i++){
			if($Balance < $levelNumber[$i])
				break;
		}
		echo '<span class="Level '.$cssClass[$i-1].'">'.$levelName[$i-1].'</span>';  
	}
...
/*
        if(!empty($MagicMetaConditions)){//propagate magic rules.
				...
				if(substr($Rule['Condition'], -1)=='%'){
					foreach(array_keys($MoreMeta) As $MoreIndex){
						if(stripos($MoreIndex,substr($Rule['Condition'], 0,-1))===0){
							...
							$RuleStep++;
							array_splice($Rules,$RuleI+$RuleStep,0,$RuleTemp);//add rule per condition
						}
					}
			...
			$RuleStep=0;
			foreach($RulesTemp As $RuleI => $Rule){
				if(substr($Rule['Condition'], -1)=='%'){//remove magic conditions
					unset($Rules[$RuleI+$RuleStep]);
					$RuleStep--;
				}
			}
			$RulesData = new Gdn_DataSet($Rules);
...
*/
        ...
        foreach($Rules As $Rule){
            ...
            if($TallyValue!=$Value){
				...
				$Options = GetValue($Rule->Operation, $this->OperationsOptions);
                $Transaction = call_user_func($this->OperationsMap[$Rule->Operation],$Value,$Target,$Condition,$User,$LastTrans,$Options);
    ...
    */
    ...
    public function Structure() {
		...
		Gdn::Structure()
			...
			->Column('LastTally','decimal(20,2)',0)
			->Set();
		...
		Gdn::Structure()
			->Table('KarmaLevelRule')
			->PrimaryKey('LevelID')
			->Column('Name','varchar(20)',0)
			->Column('KarmaNumber','int(11)',0)
			->Set();
