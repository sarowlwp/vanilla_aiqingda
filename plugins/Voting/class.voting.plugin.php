...
//			return;
		...
		<li>
			...
			echo
				...
				echo ' '.T('sorted by').' 
				<ul>
					<li'.(self::CommentSort() == 'popular' ? ' class="Active"' : '').'>'.Anchor(T('Votes'), Url('?Sort=popular', TRUE), '', array('rel' => 'nofollow')).'</li>
					<li'.(self::CommentSort() == 'date' ? ' class="Active"' : '').'>'.Anchor(T('Date Added'), Url('?Sort=date', TRUE), '', array('rel' => 'nofollow')).'</li>
				</ul>';
			?>
			</div>
		</li>
...
//			return;
      ...
      // will pick this up, compare to the browser, and up the user's
