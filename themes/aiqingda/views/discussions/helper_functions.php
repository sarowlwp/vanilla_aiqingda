...
<li class="<?php echo $CssClass; ?>">
   ...
   ?>
   <div class="StatBox ViewsBox"><span><?php echo T('Views'); ?></span><?php printf(Plural($Discussion->CountViews, '%s', '%s'), $Discussion->CountViews); ?></div>
   <div class="StatBox AnswersBox"><span><?php echo T('Comment'); ?></span><?php printf(Plural($Discussion->CountComments, '%s', '%s'), $Discussion->CountComments); ?></div>
   <div class="CommentInfo"><?php 
            if ($Discussion->LastCommentID != '') {
               echo '<span class="LastCommentBy">'.sprintf(UserAnchor($Last)).'</span>';
               echo '<span class="LastCommentDate">'.TimeAgo($Discussion->LastDate).'</span>';
			   //echo '<span class="LastCommentDate">'.Gdn_Format::Date($Discussion->LastDate).'</span>';
            } else {
               echo '<span class="LastCommentBy">'.sprintf(UserAnchor($First)).'</span>';
               echo '<span class="LastCommentDate">'.TimeAgo($Discussion->FirstDate);
...
function TimeAgo($DateTime = false) {
	$Ret = '';
	if($DateTime) {
		$Difference = time() - Gdn_Format::ToTimestamp($DateTime);
		$Ret = Gdn_Format::Seconds($Difference);		
		$Ret .= T(' ago');
	}
	return $Ret;
}
...
function WriteFilterTabs(&$Sender) {
   ...
   $Id = property_exists($Sender, 'Category') ?$Sender->Category->CategoryID : '';
   if ($Title == '')
...
<div class="Tabs DiscussionsTabs">
   <ul>
       <?php 
			echo "<li style='padding: 7px 1px;' class=\"$CssClass\">".Anchor('发布新帖', '/post/discussion/'.$Id.'', 'TabLink').'</li>';
	   ?>
   ...
   if ($Breadcrumbs) {
	  ...
	  echo '<hr>';
	  echo '<span class="CategroyDesc">', $Breadcrumb['Description'], '</span>';
   }
   if (!property_exists($Sender, 'CanEditDiscussions'))
